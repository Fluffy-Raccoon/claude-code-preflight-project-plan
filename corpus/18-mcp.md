...
export const MCPServersTable = ({platform = "all"}) => {
  ...
  return <>
    ...
    return {server.documentation ? <a href={server.documentation}>
...
3. **Register an OAuth app with the server**
  ...
  4. **Add the server with your credentials**
...
8. **Import servers from Claude Desktop**
   ```bash  theme={null}
       # Basic syntax 
       claude mcp add-from-claude-desktop 
       ```

  9. **Select which servers to import**
   After running the command, you'll see an interactive dialog that allows you to select which servers you want to import.

  10. **Verify the servers were imported**
   ```bash  theme={null}
       claude mcp list 
       ```
...
11. **Configure MCP servers in Claude.ai**
  ...
  12. **Authenticate the MCP server**
   Complete any required authentication steps in Claude.ai.

  13. **View and manage servers in Claude Code**
   In Claude Code, use the command:
...
14. **List available resources**
   Type `@` in your prompt to see available resources from all connected MCP servers. Resources appear alongside files in the autocomplete menu.

  15. **Reference a specific resource**
  ...
  16. **Multiple resource references**
...
17. **Discover available prompts**
   Type `/` to see all available commands, including those from MCP servers. MCP prompts appear with the format `/mcp__servername__promptname`.

  18. **Execute a prompt without arguments**
   ```
       > /mcp__github__list_prs
       ```

  19. **Execute a prompt with arguments**
